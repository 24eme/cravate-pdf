...
<div class="col-6 offset-3 mt-3 justify-content-center">
  ...
  <?php if ($field->getType() == \PDF\PDFFormField::TYPE_TEXT): ?>
    <div class="form-floating mb-3">
      <input type="text" class="form-control" name="<?php echo $field->getId() ?>" id="<?php echo $field->getId() ?>" <?php if($field->isRequired()): ?>required="required"<?php endif; ?>>
      <label for="<?php echo $field->getId() ?>"><?php echo $field->getLabel() ?></label>
    </div>
  <?php endif; ?>
  <?php if ($field->getType() == \PDF\PDFFormField::TYPE_SELECT): ?>
    <div class="form-floating mb-3">
      <select class="form-select" name="<?php echo $field->getId() ?>" id="floatingSelect" aria-label="<?php echo $field->getLabel() ?>" <?php if($field->isRequired()): ?>required="required"<?php endif; ?>>
  ...
  <?php if ($field->getType() == \PDF\PDFFormField::TYPE_RADIO): ?>
    <div class="mb-3">
      ...
      <div class="form-check">
        <input class="form-check-input" name="<?php echo $field->getId() ?>" value="<?php echo $choice ?>" id="option_<?php echo $choice ?>" type="radio" <?php if($field->isRequired()): ?>required="required"<?php endif; ?>>
  ...
  <?php if ($field->getType() == \PDF\PDFFormField::TYPE_CHECKBOX): ?>
    <div class="mb-3">
      ...
      <?php foreach($field->getChoices() as $choice): ?>
      <div class="form-check">
        <input class="form-check-input" name="<?php echo $field->getId() ?>" id="option_<?php echo $choice ?>" type="checkbox" <?php if($field->isRequired()): ?>required="required"<?php endif; ?>>
        <label class="form-check-label" for="option_<?php echo $choice ?>">
          <?php echo $choice ?>
        </label>
      </div>
