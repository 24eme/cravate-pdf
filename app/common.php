...
if (Config::getInstance()->get('session')) {
    ...
    $credentials = (isset($_SESSION[$f3->get('config')->get('session.credentials_key')]) && is_array($_SESSION[$f3->get('config')->get('session.credentials_key')]))? $_SESSION[$f3->get('config')->get('session.credentials_key')] : [];
